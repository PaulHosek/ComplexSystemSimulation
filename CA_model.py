...
class CA_model:
    ...
    def melt_rate(self):
        ...
        :return: m
        """
        return np.where(self.h>self.h_max,1+ self.m_p/self.m_i, (1+ self.m_p/self.m_i * self.h/self.h_max)) * self.m_i
    ...
    def rebalance_floe(self):
        '''
        rebalance the floe by calculating the change in freeboard and updating it
        '''
        dHt = (((self.H.mean()-self.h.mean()) / (self.rho_ice / self.rho_water +1)) - self.Ht.mean()) # change in Ht for the entire floe due to rebalancing
